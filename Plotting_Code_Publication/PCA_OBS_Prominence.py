...
if __name__ == '__main__':
    ...
    for cond_name in cond_names:
        ...
        for subject in subjects:
            ...
            plt.xlim([-0.05, 0.1])
            if cond_name == 'tibial':
                plt.axvline(x=22 / 1000, color='k', linewidth=0.5, label=None)
                plt.title(f"Tibial Nerve Stimulation\n"
                          f"Lumbar Spinal Cord")
            elif cond_name == 'median':
                plt.axvline(x=13 / 1000, color='k', linewidth=0.5, label=None)
                plt.title(f"Median Nerve Stimulation\n"
                          f"Cervical Spinal Cord")

            plt.axvline(x=0 / 1000, color='r', linewidth=0.5, label=None)
            fname = f"{subject_id}_{cond_name}_pca_obs.png"
