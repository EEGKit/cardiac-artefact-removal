...
def fit_ecgTemplate_tukey(data, pca_template, aPeak_idx, peak_range, pre_range, post_range, baseline_range, midP, fitted_art, post_idx_previousPeak, n_samples_fit, plot_tukey):
    ...
    # select window of template, seems to select the whole PCA template
    # Need to minus 1 so it indexes from 0, need to add 1 as python doesn't include last value
    ...
    # MATLAB backslash apparently is equivalent to this in python linalg.lstsq(x,y)
    # MATLAB * operator is dot product in python, python * is performed element wise - can't use
    least_square = np.linalg.lstsq(template, detrended_data, rcond=None)
    pad_fit = np.dot(template, least_square[0])

    # fit artifact, I already loop through externally channel to channel to no need to include it here
    # Trouble with indices here
    # Original Line
    ...
    if plot_tukey:
        plt.figure()
        plt.plot(pad_fit[midP - pre_range-1: midP + post_range].T, label='Before')
        plt.legend()
        plt.figure()
        plt.plot(tukey_window, label='Window')
        plt.legend()
        plt.figure()
        plt.plot(fitted_art[0, aPeak_idx[0] - pre_range-1: aPeak_idx[0] + post_range], label='After')
        plt.legend()
        plt.show()
    ...
    if len(post_idx_previousPeak) != 0:
        ...
        # intpol_window = np.ceil([post_idx_previousPeak[0], aPeak_idx[0] - pre_range-1]).astype('int')
        ...
        if intpol_window[0] < intpol_window[1]:
            ...
            # Everything I'm doing should be 1D since I don't take in multiple channels at a time
            fitted_art[0, post_idx_previousPeak[0]: aPeak_idx[0] - pre_range+1] = y_interpol
            # fitted_art[0, post_idx_previousPeak[0]: aPeak_idx[0] - pre_range] = y_interpol
