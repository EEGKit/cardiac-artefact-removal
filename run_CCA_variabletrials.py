# Script to actually run CCA on the data - using smaller trial numbers
...
def run_CCA_variabletrials(subject, condition, srmr_nr, data_string, n, trial_indices):
    plot_graphs = False
    ...
    epochs_all = mne.Epochs(raw, events, event_id=event_id_dict, tmin=iv_epoch[0], tmax=iv_epoch[1]-1/1000,
                        baseline=tuple(iv_baseline), preload=True)
    epochs = epochs_all[trial_indices]  # Select epochs of interest
