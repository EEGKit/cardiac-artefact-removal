...
# Plot both spinal and cortical as inset to ECG
...
if __name__ == '__main__':
    ...
    for cond_name in cond_names:
        ...
        for channel_type in channel_types:  # Conditions (median, tibial)
            evoked_list = []
            ...
            if cond_name == 'tibial':
                ...
                if channel_type == 'ECG':
                    ...
                    channel = ['L1']
                    ax = axis.inset_axes([0.7, 0.7, 0.25, 0.25])
                    # ax = inset_axes(axis, width='40%', height='35%', loc=1, borderpad=1)
                elif channel_type == 'Cortical':
                    channel = ['Cz']
                    ax = axis.inset_axes([0.7, 0.30, 0.25, 0.25])
                ...
                if channel_type == 'ECG':
                    ...
                    channel = ['SC6']
                    ax = axis.inset_axes([0.7, 0.70, 0.25, 0.25])
        ...
        cb = fig.colorbar(ax.images[-1], cax=cbar_ax)
        # cb = im[-1].colorbar
        cb.set_label('Amplitude (dB)')
        fname = f"CardiacTFRInset_{cond_name}_dB_SAB.png"
        fig.savefig(image_path+fname)
        plt.savefig(image_path+f"CardiacTFRInset_{cond_name}_dB_SAB.pdf", bbox_inches='tight', format="pdf")
        plt.clf()
    plt.show()
